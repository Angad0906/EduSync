...
export class PerformanceMonitor {
  ...
  monitorFPS(callback, duration = 5000) {
    ...
    const measureFrame = (currentTime) => {
      ...
      if (elapsed >= duration) {
        const fps = Math.round((frames * 1000) / elapsed);if (callback) callback(fps);
        return;
      }
  ...
  monitorPaintTiming() {
    ...
    if ('PerformanceObserver' in window) {
      const lcpObserver = new PerformanceObserver((list) => {
        ...
        const lastEntry = entries[entries.length - 1];}ms`);
      });
  ...
  monitorLongTasks() {
    ...
    const longTaskObserver = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {}ms`);
      }
    });
  ...
  generateReport() {
    ...
    console.table(report.metrics);console.groupEnd();

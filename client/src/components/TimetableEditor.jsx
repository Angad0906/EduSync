...
function TimetableEditor() {
  ...
  const fetchScheduleDetails = async (scheduleId) => {
    ...
    try {
      ...
      setHasChanges(false);
      setError("");
    } catch (error) {setError("Error fetching schedule details. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchCourses = async () => {
    ...
    } catch (error) {toast.error("Error fetching courses. Please try again.");
    }
  };
  ...
  const fetchRooms = async () => {
    ...
    } catch (error) {toast.error("Error fetching rooms. Please try again.");
    }
  };
  ...
  const handleDragEnd = (event) => {
    ...
    const [sourceDay, sourceTime] = active.id.split("-");
    const [targetDay, targetTime] = over.id.split("-");// Find the source slot

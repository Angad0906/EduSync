...
export async function generateSchedule(
  ...
  let extraSessions = availableSlots - sessionsPerCourse * courseGroupCount
...
function createFairSchedule(courses, rooms, teachers, year, branch, division, opts) {
  ...
  if (suitableRooms.length === 0) {return schedule
  }
  
  // First, schedule lab sessions (they need 2 consecutive slots)
  const labRooms = rooms.filter(room => room.type === "lab" && room.isAvailable)
  if (labRooms.length > 0 && labCourses.length > 0) {for (const labCourse of labCourses) {
  ...
  // Log the distributionsubjectDistribution.forEach(subject => {})
          return schedule
}

// Simple fair scheduling algorithm that ensures equal distribution
function createSimpleFairSchedule(courses, rooms, teachers, year, branch, division) {// Get all theory courses
  const theoryCourses = courses.filter(c => c.lectureType === "theory")
  const labCourses = courses.filter(c => c.lectureType === "lab")// Create all possible time slots
  const allSlots = []
  for (const day of DAYS) {
    for (const timeSlot of TIME_SLOTS) {
      if (timeSlot !== LUNCH_BREAK_SLOT) {
  ...
  const extraSlots = allSlots.length % theoryCourses.length// Create a round-robin distribution
  const schedule = []
  const suitableRooms = rooms.filter(room => 
    (room.type === "classroom" || room.type === "lecture-hall") && 
    room.isAvailable
  ...
  // Log the final distributionconst distribution = {}
...
function createSystematicSchedule(courses, rooms, teachers, year, branch, division) {
  ...
  // Step 3: Systematically assign theory courses to remaining slotslet courseIndex = 0
  let roomIndex = 0
  ...
  // Step 4: Log final distributioncourseDistribution.forEach(item => {})
          return schedule
}
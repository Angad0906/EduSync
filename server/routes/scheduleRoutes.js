...
router.get("/latest", async (req, res) => {
    try {
        ...
        // Log the received parameters// Validate the parameters
...
router.post("/courses", async (req, res) => {
    try {
        const { name, code, instructor, duration, capacity, year, branch, division, lectureType, preferredTimeSlots } =
            req.body// Check if the instructor is allowed to teach this year
        ...
        catch (error) {if (error.name === "MongoServerError" && error.code === 11000) {
...
router.post("/teacher-availability", async (req, res) => {
    try {
        const { teacherId, availability } = req.body// First, update the teacher's availability directly
        ...
        catch (emailError) {// Continue execution even if email fails
        }
...
router.get("/conflicts", async (req, res) => {
    try {
        ...
        schedule.timetable.forEach((slot, index) => {
            ...
            // Check room conflicts (only if room exists)
        if (slot.room && slot.room._id) {
            ...
            // Check instructor conflicts (only if instructor exists)
        if (slot.course && slot.course.instructor && slot.course.instructor._id) {
